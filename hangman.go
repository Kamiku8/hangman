	...
	for {
		...
		if isALetter == false {
			...
			if has_won == true {
				...
				fmt.Printf("Tu a gagner! Le mots est %s\n", word)
				for {
					fmt.Printf("Play again? (y/n) \n")
					fmt.Scanln(&again)
					isYorN, somekindoferror := regexp.MatchString("^y|Y|n|N", again)
					if somekindoferror != nil {
						fmt.Printf("Quelque chose a horriblement mal tourné. ")
						fmt.Printf("existant avec erreur ne peut pas correspondre @ l'expression régulière %v", again)
						return
					}
					if isYorN == false {
						fmt.Printf("Try again\n")
					} else if len(again) > 1 {
						fmt.Printf("YTry again\n")
					} else if strings.ToLower(again) == "y" {
						return "y", true
					} else {
						return "n", true
